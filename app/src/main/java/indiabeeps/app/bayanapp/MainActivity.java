...
//    fontSize = 100;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GeneralFunction.bNightMode = prefs.getBoolean(Splash.sNightMode, false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.miNightmode) {
            if (item.isCheckable()) {
                ...
                editor.putBoolean(Splash.sNightMode, IsNightMode);
                editor.apply();
                RestartActivity();
            }
        }
        ...
        if (id == R.id.action_speed_plus) {
            if (isScrollingNow) {
                if (GeneralFunction.SCROLL_SPEED > 7) {
                    ...
                    editor.putInt(Splash.sSCROLL_SPEED, GeneralFunction.SCROLL_SPEED);
                    editor.apply();
                    //Toast.makeText(getApplicationContext(), "Scroll Speed +", Toast.LENGTH_SHORT).show();
                }
            }
            return true;
        }
...
//                final ArticlesDB getCats = new ArticlesDB(this);
//                getCats.updateNote(myID);
            return true;
        }
